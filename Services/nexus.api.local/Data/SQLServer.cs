...
namespace nexus.common.dal
{
    public partial class SQLServer : IDisposable
    {
        ...
        private string  mConnectionStringUsername = "Data Source={0};Initial Catalog={1};User ID={2};Password={3};Trusted_Connection=False;Connection Timeout=600;Encrypt=false";
            ...
            if (Connect())
            {
                using (SqlCommand cmd = CreateCommand("", SQL))
                {
                    try
                    {
                        ...
                        {
                            dt.TableName = "MSSQL";
                            using (DataReaderAdapter dra = new DataReaderAdapter())
                                dra.FillFromReader(ref dt, dr);
                        }
                    }
                    catch (Exception ex) { mDataError = ex.Message; }
                }
                ...
                if (!String.IsNullOrEmpty(Param))
                {
                    ...
                    if (!String.IsNullOrEmpty(VarArr.GetValue(2).ToString()))
                    {
                        try
                        {
                            ...
                            {
                                case "I":
                                case "N": if (ParamValue.Contains(".")) ParamValue = Convert.ToDouble(ParamValue).ToString("0");
                                ...
                                case "D": prm.DbType = DbType.DateTime; prm.Value = Convert.ToDateTime(ParamValue);                                           break;
        ...
        public bool UpdateImage(byte[] FaceImage, string ImageId)
        {
            ...
            using (SqlConnection CN = new SqlConnection("")) //dwr shell.Instance.ConnStr))
            {
        ...
        //public BitmapImage ImageSourcefromRow(DataRow Row, string Field)
        //{
        //    return BitmapImagefromRow(Row, Field);
        //}

        //public BitmapImage BitmapImagefromRow(DataRow Row, string Field)
        //{
        ...
        //        if (Row[Field] != null)
        //        {
        //            byte[] blob = (byte[])Row[Field];
        //            if (blob.Length > 0)
        //            {
        //                System.IO.MemoryStream lstr = new System.IO.MemoryStream(blob);

        //                BMI.BeginInit();
        //                BMI.StreamSource = lstr;
        //                BMI.EndInit();
        //            }
        //        }
        //    }
        //    catch (Exception ex)
        //    {
        //        //Props.Instance.InsertLogEntry(true, Zones.Instance.ZoneId.ToString(), "designer", "element", "create", "", "image error " + ex.Message);
        //    }
        ...
        public int InsertEGMRequest(string Request, int EventPK, bool Active, int EgmID, int MemberID, int GamingTypeUI, int ActionTypeID, string Property, string Description)
        {
            ...
            if (Active)
            {
                try
                {
                    ...
                    SQL += "update [gam].[Current] set " + Request + "=@EventPK, " + Request + "Start='" + DateTime.Now.ToString("MMM dd, yyyy HH:mm") + "' where EgmID=" + EgmID;
                    SQL += "Select @EventPK ";
        ...
        public static string DateToString(Object obj) {return DateToString(obj, "MMM dd, yyyy HH:mm:ss.fff");}
