...
{
    public class PoolHelper : IDisposable
    {
        public async Task<PoolResponse> Process(PoolRequest request)
        {

            PoolResponse Items = new PoolResponse();
            ...
            if (await TokenHelper.GetToken())
            {
                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    // Send a POST request to check for Pools
                    var response = await httpClient.PostAsync("pro/Pool", content);
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        try
                        {
                            ...
                            Items = JsonSerializer.Deserialize<PoolResponse>(jsonString, new JsonSerializerOptions
                            {
