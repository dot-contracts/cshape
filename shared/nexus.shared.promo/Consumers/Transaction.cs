...
namespace nexus.shared.promo
{
    ...
    {
        public async Task<TransactionResponse> Process(TransactionRequest request)
        {

            TransactionResponse Items = new TransactionResponse();
            ...
            if (await TokenHelper.GetToken())
            {
                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    // Send a POST request to check for Transactions
                    var response = await httpClient.PostAsync("pro/Transaction", content);
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        try
                        {
                            ...
                            Items = JsonSerializer.Deserialize<TransactionResponse>(jsonString, new JsonSerializerOptions
                            {
