...
{
    public class TriggerHelper : IDisposable
    {
        public async Task<TriggerResponse> Process(TriggerRequest request)
        {

            TriggerResponse Items = new TriggerResponse();
            ...
            if (await TokenHelper.GetToken())
            {
                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    // Send a POST request to check for Triggers
                    var response = await httpClient.PostAsync("pro/Trigger", content);
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        try
                        {
                            ...
                            Items = JsonSerializer.Deserialize<TriggerResponse>(jsonString, new JsonSerializerOptions
                            {
