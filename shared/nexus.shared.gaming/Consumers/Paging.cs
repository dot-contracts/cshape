...
namespace nexus.shared.gaming
{
    public class PagingHelper : IDisposable
    {
        ...
        public async Task<List<Paging>> Process()
        {
            List<Paging> data = new();
            ...
            if (await TokenHelper.GetToken())
            {
                ...
                var requestBody = new PagingRequest
                {
                ...
                var response = await _httpClient.PostAsync("gam/Paging", content);

                if (response.IsSuccessStatusCode)
                {
                    var json = await response.Content.ReadAsStringAsync();
                    var result = JsonConvert.DeserializeObject<PagingResponse>(json);
