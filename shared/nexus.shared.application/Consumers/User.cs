...
namespace nexus.shared.application
{
    public class UserHelper : IDisposable
    {
        ...
        public async Task<DataTable> Process(string UserId="")
        {
            ...
            {
                // Now that you have the token, you can use it to check for Users
                using (var httpClient = new HttpClient())
                {
                    ...
                    var response = await httpClient.PostAsync("app/User", content);
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        // Create DataTable
                        Users = nextjson.ArrayToDataTable(jsonObject["Users"].ToArray());
                    }
                    else
                    {
                        //MessageBox.Show($"Users check failed with status code: {response.StatusCode}");
                    }
                }
            }
