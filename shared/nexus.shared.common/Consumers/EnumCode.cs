...
namespace nexus.shared.common
{
    ...
    public static class EnumCodeHelper
    {
        ...
        {
            if(!EnumCache.Instance.CacheLoaded)
            {
                ...
                if (await TokenHelper.GetToken())
                {
                    ...
                    using (var httpClient = new HttpClient())
                    {
                        ...
                        var response = await httpClient.PostAsync("/cmn/EnumCode", content);
                        ...
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            // Create DataTable
                            Enums = nextjson.ArrayToDataTable(jsonObject["enumCodes"].ToArray());
                        }
                        else
                        {
